...
namespace hpx {
    namespace io {
        namespace server {
            ...
            ssize_t write(std::vector < char >
            const& buf) {
            ssize_t result = 0;
        {
            hpx::parallel::execution::io_pool_executor scheduler;
            scheduler.
            add(hpx::util::bind(&local_file::write_work,
    ...
    void write_work(std::vector < char >
    const& buf,
    ssize_t &result
    ) {
    if (fp_ == NULL || buf.
...
ssize_t pwrite(std::vector < char >
const& buf,
off_t const offset
)
{
...
hpx::parallel::execution::io_pool_executor scheduler;
scheduler.
add(hpx::util::bind(&local_file::pwrite_work,
...
void pwrite_work(std::vector < char >
const& buf,
off_t const offset, ssize_t
& result)
{
if (fp_ == NULL || buf.
